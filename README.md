...
Die Anwendung registriert den CORS-Service-Provider nur, wenn das Paket vorhanden ist. Nach einer erfolgreichen Installation verschwindet der Fehler. Sollte Ihr Hosting-Anbieter die Funktion `highlight_file` deaktiviert haben und Sie weiterhin eine Debug-Seite sehen, setzen Sie `APP_DEBUG=false` in Ihrer `.env`, um die Laravel-Fehlerseite zu deaktivieren.
